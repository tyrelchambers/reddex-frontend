...
const StorySubmission = inject(
  "FormStore",
  "SiteStore"
)(
  observer(({ FormStore, SiteStore }) => {
    ...
    return (
      <div className={`static-submit-wrapper ${SiteStore.config.theme}`}>
        ...
        {SiteStore.config.headline && (
          <h3 className="mt-6 headline">{SiteStore.config.headline}</h3>
        )}
        <div
          dangerouslySetInnerHTML={{ __html: SiteStore.config.rules }}
          className="mt-6 mb-6"
          id="preview-body"
          style={{ whiteSpace: "pre-line" }}
        ></div>
        <SubmissionForm data={FormStore.state} />
