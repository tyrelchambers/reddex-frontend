...
const Approved = ({ list, callback, ModalStore, ReadingListStore }) => {
  ...
  const Story = ({ x }) => (
    <li className="reading-list-item-wrapper cell-row">
      <div className="reading-list-item grid grid-cols-5 gap-4 grid-flow-col ai-c">
        <a
          href={x.url}
          className="reading-list-title ellipses w-100pr font-bold col-span-2"
          target="_blank"
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="td-n td-u-hv reading-list-author ellipses"
          style={{ color: "inherit" }}
        >
          {x.author}
        </a>
        <p className="tt-c">{x.subreddit}</p>
        <p className="reading-time">{avgReadingTime(x.self_text)} minutes</p>
        <Dropdown
      ...
      <div className="tag-list d-f ai-c" id="tagList">
        {x.Tags
          ? x.Tags.map((t, id) => {
              return (
                <p
                  className="subtle d-f tag-small"
                  key={id}
    ...
    return (
      <React.Fragment key={id}>
        {readingList.map((y, id) => {
          if (x === y.subreddit) {
            return <Story key={`${y.post_id}_${id}`} x={y} story_id={id} />;
          }
        })}
      </React.Fragment>
    );
  ...
  return (
    <>
      ...
      <ul className="reading-list-list mt-">{renderedList}</ul>
    </>
  );
};
